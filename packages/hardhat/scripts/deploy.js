...
const main = async () => {
  ...
  // const usdtContract = await deploy("TetherToken", [
  //   amount,
  //   "USDT",
  //   "USDT",
  //   18,
  ...
  const balanceOwner = await usdtContract.balanceOf(signer.getAddress());
  console.log(balanceOwner.toString());

  const receiver = "0x8ef9F0acfEF3D9AB023812BB889A8F5a214B9B82";
  await usdtContract.transfer(receiver, amount);
  const balance = await usdtContract.balanceOf(receiver);
