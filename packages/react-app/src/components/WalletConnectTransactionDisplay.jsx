...
export default function WalletConnectTransactionDisplay({ payload, chainId, currentlySelectedChainId }) {
      ...
      try {
        ...
        const body = {
          network_id: params.chainId
            ? BigNumber.from(params.chainId.toString()).toNumber()
            : BigNumber.from(chainId.toString()).toNumber(), // Tenderly doesn't like hex
          from: params.from,
          to: params.to,
          input: params.data ? params.data : "",
          gas: params.gasLimit
            ? BigNumber.from(params.gasLimit).toNumber()
            : params.gas
    ...
    for (let i = 0; i < paramsArray.length; i++) {
      ...
      if (label == "From" || label == "To") {
        ...
        mainnetProvider
          ...
          .then(ensName => {
            if (ensName) {
              paramsArray[i] = { label, value: ensName };
  ...
  const getColoredNetworkName = chainId => {
    ...
    return <span style={{ color: networkColor }}>{networkName}</span>;
  };
  ...
  const networkWarning = (
    <div key="network" style={{ display: "flex", justifyContent: "center", marginTop: "0.5em", marginBottom: "0.5em" }}>
      <div style={{ textAlign: "center" }}>
        You have {getColoredNetworkName(currentlySelectedChainId)} selected,
        <br />
        but this is a {getColoredNetworkName(chainId)} request!
      </div>
    </div>
  );
  ...
  try {
    if (!payload || !payload.params) {
      return <div>Cannot decouple payload.</div>;
    }
    ...
    if (paramsArray.length > 0) {
        ...
        if (param.value) {
          ...
          options.push(
            <div
              ...
              style={{ display: "flex", justifyContent: "center", marginTop: "0.5em", marginBottom }}
            >
              <div style={{ color: "grey" }}> {param.label}:</div>{" "}
              <div style={{ fontWeight: "bold" }}> {param.value}</div>
            </div>,
          );
        }
      });
      ...
      return (
        <pre>
          ...
          <div style={{ display: "flex", flexDirection: "column", justifyContent: "space-around" }}>{options}</div>
        </pre>
      );
    }
    ...
    let params;
    const param_0 = payload.params[0];
    const param_1 = payload.params[1];
